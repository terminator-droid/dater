...
`GET /matches/{id}` объединяет данные из двух источников: **PG** (match + план партнёра с `drink_tonight`) + **MongoDB** (полный текст тем по `topic_ids`). Имя и возраст партнёра по-прежнему скрыты.
